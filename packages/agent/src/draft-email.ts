...
const MERCHANT_FAULT_EXEMPLAR = `Subject: We could not process your Acme payment
Body: Hi Sam, we were not able to process your INR 2400.00 payment because of a settings issue on our side - some cards, such as international ones, are not accepted by our current payment setup. Nothing is wrong with your card or your account. You can complete the payment here: https://example.com/pay/xyz, and trying a different card or payment method may work. If you run into any trouble, just reply to this email. Unsubscribe: https://example.com/unsub/xyz`;

const MERCHANT_FAULT_EXEMPLAR_HINGLISH = `Subject: Aapka Acme payment process nahi ho paya
Body: Hi Sam, hamari side ki ek settings ki wajah se aapka INR 2400.00 ka payment process nahi ho paya - kuch cards, jaise international wale, hamare current payment setup mein accept nahi hote. Aapke card ya account mein koi problem nahi hai. Payment yahan complete kar sakte hain: https://example.com/pay/xyz, aur dusra card ya method try karne se ho sakta hai. Koi dikkat ho to is email ko reply karein. Unsubscribe: https://example.com/unsub/xyz`;

function exemplarFor(facts: CaseFacts): string {
  const prefix = "Example of the tone and shape wanted (different case, do not copy its details):\n\n";

  if (facts.failureSource === "business") {
    const sample = facts.language === "hinglish" ? MERCHANT_FAULT_EXEMPLAR_HINGLISH : MERCHANT_FAULT_EXEMPLAR;
